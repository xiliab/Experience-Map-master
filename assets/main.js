...
template: `
  <div class="modal-bg"
    ...
    <button
      ...
      style="position:absolute;left:2vw;top:50%;transform:translateY(-50%);font-size:32px;background:rgba(0,0,0,.5);color:#fff;border:none;border-radius:50%;width:40px;height:40px;cursor:pointer;z-index:11;">
      ‹
    </button>
    <button
      @click="nextImage"
      :disabled="currentIndex === images.length-1"
      style="position:absolute;right:2vw;top:50%;transform:translateY(-50%);font-size:32px;background:rgba(0,0,0,.5);color:#fff;border:none;border-radius:50%;width:40px;height:40px;cursor:pointer;z-index:11;">
      ›
    </button>
    <div style="display:flex;flex-direction:column;align-items:center;">
      <div ref="modalCard"
        class="modal-content"
        style="position:relative;background:#fff;padding:24px;border-radius:12px;box-shadow:0 8px 32px rgba(0,0,0,0.25);max-width:80vw;max-height:80vh;overflow:auto;display:flex;flex-direction:column;align-items:center;">
        <div style="max-width:70vw;max-height:60vh;display:flex;align-items:center;justify-content:center;">
          <img v-if="!current.type || current.type === 'image'" :src="current.src" style="max-width:70vw;max-height:60vh;object-fit:contain;border-radius:8px;background:#eee;" />
          <video v-else-if="current.type === 'video'" :src="current.src" style="max-width:70vw;max-height:60vh;border-radius:8px;background:#000;" controls autoplay></video>
        </div>
        <!-- 这里插入圆点指示器 -->
...
computed: {
  ...
  closeBtnStyle() {
    return `
      position:fixed;
      left:${this.closeBtnLeft};
      top:${this.closeBtnTop};
      transform:${this.closeBtnTransform};
      z-index:12;
      padding:8px 36px;
      font-size:18px;
      border:none;
      ...
      },
      {
        ...
        imgs: [
          ...
          },
          {
            ...
            thumb: "https://img.xiliab.online/uploads/3D_print.jpg", // 你的视频封面
...
• 面向家装设计师和 DIY 用户，门槛低、操作简便。`,
  ...
  methods: {
    ...
    openModal: function (type, idx = 0) {
      this.modalType = type;
  this.currentImageIndex = idx; // 记录下标
  this.showModal = true;
    },
    openAbout: function() {
